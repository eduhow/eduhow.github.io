  ...
  return (
    <div className={`w-full ${className}`}>
      <div 
        ...
        className={`group relative flex flex-col items-center justify-center w-full min-h-[200px] bg-slate-50/50 border-2 border-dashed border-slate-300 hover:border-slate-400 hover:bg-slate-100/50 cursor-pointer ${isHighlighting ? "highlight-active" : "transition-colors duration-300"}`}
        style={isHighlighting ? { transition: 'none' } : undefined}
      >
        <Button
          size="sm"
          className={`${UPLOAD_BTN_BG} ${UPLOAD_BTN_HOVER} text-white border-transparent cursor-pointer rounded-none gap-2 px-6 h-10 shadow-sm opacity-100${isHighlighting ? "" : " transition-all duration-300"} group/uploadbtn`}
          style={isHighlighting ? { transition: 'none' } : undefined}
        >
          <Upload className={`size-4${isHighlighting ? "" : " transition-transform duration-300"}${isHighlighting ? "" : " group-hover/uploadbtn:animate-bounce-slow"}`} style={isHighlighting ? { transition: 'none' } : undefined} />
          Soru yükle
        </Button>
      </div>
      ...
      <TooltipProvider>
        <Tooltip open={showResizeTooltip}>
          <TooltipTrigger asChild>
            <div
              className={`no-print h-2 w-full cursor-s-resize bg-zinc-100 hover:bg-zinc-300 rounded-b print:hidden flex items-center justify-center group/resize ${isHighlighting ? "highlight-active" : "transition-colors duration-300"}`}
              style={isHighlighting ? { transition: 'none' } : undefined}
